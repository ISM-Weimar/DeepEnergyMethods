...
Implement a Poisson 2D problem with Dirichlet and Neumann boundary conditions:
    ...
    u(x,y) = 0, for x = 0
    du/dy = 0 for y = 0, y = 1
    du/dx = pi*cos(pi*x)*cos(pi*y) for x = 1
    
    Exact solution: u(x,y) = sin(pi*x)*cos(pi*y) corresponding to 
...
class PhysicsInformedNN:
    ...
    def __init__(self, X_b_left, u_b_left, X_b_bottom, u_y_b_bottom, X_b_top, u_y_b_top, X_b_right, u_y_b_right, X_f, layers, lb, ub):
...
#       
    ...
    def net_f_u(self,x,y):
        ...
        f_u = u_xx + u_yy + 2*pi**2*tf.sin(pi*x)*tf.cos(pi*y)
...
if __name__ == "__main__":
    ...
    y_right = np.array([np.linspace(0.0,1.0,int(N_b/4))], dtype = np.float32)
    X_b_right = np.concatenate((x_right, y_right.T), axis=1)
    x_right_scal = 1;
    u_x_b_right = np.float32(np.pi*np.cos(np.pi*x_right_scal)*np.cos(np.pi*y_right))
    ...
    model = PhysicsInformedNN(X_b_left, u_b_left, X_b_bottom, u_y_b_bottom,  X_b_top, u_y_b_top, X_b_right, u_x_b_right, X_f, layers, LB_Plate, UB_Plate)
    ...
    u_exact = np.sin(np.pi*x_grid)*np.cos(np.pi*y_grid)
